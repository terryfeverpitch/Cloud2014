package com.cloud2014.assignment.client.page;

import java.util.ArrayList;
import java.util.Date;

import com.cloud2014.assignment.client.Cloud2014_assignment;
import com.cloud2014.assignment.server.Account;
import com.cloud2014.assignment.shared.AccountDTO;
import com.cloud2014.assignment.shared.FieldVerifier;
import com.cloud2014.assignment.shared.MessageCode;
...
public class HomePage extends VerticalPanel {
	...
	public void user_homepage() {
		...
		MenuItem mntmDrive = new MenuItem("Drive", false, new Command() {
			public void execute() {
				Cloud2014_assignment.accountService.getAccount(username, new AsyncCallback<AccountDTO>() {
					@Override
...
//		buildInfoPage();
	...
	public void admin_homepage() {
		...
		MenuItem item = new MenuItem("Datastore viewer", false, new Command() {
			@Override
			public void execute() {
				Cloud2014_assignment.accountService.getAccountList(new AsyncCallback<ArrayList<AccountDTO>>() {
					@Override
					public void onFailure(Throwable caught) {
			...
			public void onClick(ClickEvent event) {
				...
				if(src == btn_delete) {
					...
					if(!Window.confirm(msg))
						return;
					Cloud2014_assignment.accountService.deleteAccount(account.getUsername(), new AsyncCallback<Integer>() {
						@Override
						public void onFailure(Throwable caught) {	
					...
					if(btn_block.getText().equals("block"))
						...
						available = true;
					Cloud2014_assignment.accountService.blockOrFreeAccount(account.getUsername(), available, new AsyncCallback<Integer>() {
						@Override
						public void onFailure(Throwable caught) {	
							Window.alert("failure");
						}
	...
	public class mClickHandler implements ClickHandler {
		...
		public void onClick(ClickEvent event) {
			...
			if(src == signPage.btn_add) {
				...
				Cloud2014_assignment.accountService.updateAccount(account, new AsyncCallback<Integer>() {
					@Override
					public void onFailure(Throwable caught) {	
